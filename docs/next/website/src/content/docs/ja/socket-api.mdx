...
生の `agent.prompt` リクエストには、`agent.get` または `session.snapshot` が返す `source`、`agent`、`kind`、`value` を持つ `expected_session` を指定できます。Herdr は、プロンプトを送信する同じアプリ操作内で、ネイティブセッション識別子全体を比較します。セッションが存在しないか異なる場合、ターミナル入力を送信する前に `agent_session_mismatch` を返します。古いサーバーは未知のリクエストフィールドを無視する可能性があるため、このフィールドに依存する前に、クライアントは `ping.result.capabilities.agent_prompt_expected_session == true` を必須とする必要があります。
