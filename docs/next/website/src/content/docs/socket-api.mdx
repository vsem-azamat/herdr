...
Raw `agent.prompt` requests can include `expected_session` with the `source`, `agent`, `kind`, and `value` returned by `agent.get` or `session.snapshot`. Herdr compares the complete native session identity in the same app operation that submits the prompt. A missing or different session returns `agent_session_mismatch` before terminal input is sent. Clients must require `ping.result.capabilities.agent_prompt_expected_session == true` before relying on this field because older servers may ignore unknown request fields.
