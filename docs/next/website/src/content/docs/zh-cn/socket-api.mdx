...
原始 `agent.prompt` 请求可以包含 `expected_session`,其 `source`、`agent`、`kind` 和 `value` 来自 `agent.get` 或 `session.snapshot`。Herdr 会在提交提示的同一个应用操作中比较完整的原生会话标识。会话缺失或不同时,服务器会在发送任何终端输入前返回 `agent_session_mismatch`。旧服务器可能忽略未知请求字段,因此客户端在依赖此字段前必须确认 `ping.result.capabilities.agent_prompt_expected_session == true`。
